<?php
class page_newsletter extends AWPage {
	function init(){
		parent::init();
		...
		$this->add('View_Hint')->set('Expect to see here web tips, start-up info and some information about project development. We won\'t bore you with technical details, product features or our personal stories (we have two other blogs if it is what you are looking for). We expect this newsletter to be something you can get value from.');
		$f=$this->add('AWForm');
		...
		if($f->isSubmitted()){
			...
			$f->js()->closest('.atk4_loader')
				->atk4_loader('loadURL',$this->api->getDestinationURL('newsletter.html',array(
								'm'=>$result->Message
								)))
				->execute();
